...
#%%
Network = 'synthetic_small'

# paths to necessary files/tools/software. Please change accordingly.

# SPSA algorithm parameters setup. Please refer to the slides the meaning of these parameters.
    
# objective = counts or travel_times
# Note: travel_times is not available for SiouxFalls
...
#%%
start = time.time()
Best_RMSN, Best_OD, Best_SimulatedCounts, rmsn, list_ak, list_ck, list_g = \
SPSA.SPSA(config, sim_setup, spsa_setup, true_data, input_od)
end = time.time()
print('Running time: %d' %(end-start))
plt.rcParams.update({'figure.figsize':(6,6), 'figure.dpi':60, 'figure.autolayout': True})
plt.figure()
plt.title('Calibrating '+Network+' with SPSA')

...
import time
import os
from Functions import load_path, load_data, write_od, RSUMOStateInPar, SUMO_aver, gof_eval, create_Dm, due_RUN
...
if __name__ == '__main__':
    ...
    start = time.time()
    # load path and data
    Paths, Network_var, SPSA_para, PC_SPSA_para, Supply_para, Route_choice_para = load_path(user, system, True, 'counts', 'create', 10, 1, 1)
...
#                due_RUN(Paths, Network_var, Route_choice_para, 0, np.random.normal(0,10000), 'create') # later have to add logit            
